...
b <- sqrt(area)
```
Here, the value of `area` is given to the `sqrt()` function, the `sqrt()` function
calculates the square root, and returns the value. If we want to use that value
later, we can assign it to the object `b`. 
...
may be specified by the user, or, if left out, take on a *default* value:
...
This is considered fairly risky practice, because it relies on you knowing 
the exact order of the arguments. 
...
A vector is a very common and basic data type in R.
